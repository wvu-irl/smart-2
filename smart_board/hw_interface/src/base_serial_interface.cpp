...
bool base_classes::base_serial_interface::startWork()
{
    if(!interfaceReady())
    {
        timer.reset(new boost::asio::deadline_timer(interfacePort->get_io_service()));
		timer->expires_from_now(boost::posix_time::seconds(10));
		timer->async_wait(boost::bind(&base_classes::base_serial_interface::timeoutHandler,this,boost::asio::placeholders::error()));
		return true;
    }
    if(!interfaceStarted)
    {
...
bool base_classes::base_serial_interface::handleRegexRequest(const boost::system::error_code& e, std::size_t bytesTransferred)
{
  ...
  ROS_DEBUG("Thread <%s>:: %s:: Received Packet!:: Size %lu", THREAD_ID_TO_C_STR, this->pluginName.c_str(), bytesTransferred);
    ...
    {
        ROS_ERROR("Plugin Error Occurred in plugin data handler <%s>", this->pluginName.c_str());
    }
...
void base_classes::base_serial_interface::timeoutHandler(const boost::system::error_code& error)
{
    if(!error)
    {
        ROS_DEBUG("error Timer finished");
        startWork();
        return;
    }
    ROS_ERROR("Timer error %s", error.message().c_str());
}
